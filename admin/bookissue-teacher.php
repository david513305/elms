...
if(isset($_POST["userid"])) {
    ...
    if ($message_error == "") {
       ...
       // $teacherid=$_POST["userid"];
        ...
        for($i=0;$i<$attemptIssueBooks;$i++) {
            $stmt = $conn->prepare("Select isbn_number, piece_of_books from books where isbn_number='".$_POST['isbnnumber'][$i]."'");
            $stmt->execute();
            $row = $stmt->fetch();
            $isbn = $row['isbn_number'];
            $piece_of_books = $row['piece_of_books'];
            if($piece_of_books<1){
                //print_r("Oops! no book available with the isbn ".$isbn." in stock");exit();
                header('Location:bookissue-teacher.php?stocklimited="'.$isbn.'"', true);
                ob_flush();
                exit();
            }
        }
        ...
        if($totalPrevIssuedBooks== 3){
            header("Location:bookissue-teacher.php?book-issue-available-no=1");
            ...
            if($attemptIssueBooks>=2) { // IF Want 2/3 book since I've 1 left
                header("Location:bookissue-teacher.php?book-issue-available-1=1");
            } else {
                ...
                if($matchedCount>0){//print_r("matched");exit;
                    header("Location:bookissue-teacher.php?duplicate=1");
                } else {
                    ...
                    if($lateFine>=5){//print_r($lateFine);exit();
                        header("Location:bookissue-teacher.php?penalty=1");
                    } else {
                        if($piece_of_books>0){
                            $updatePieceOfBooks = $piece_of_books-1;
                        }
                        $sql = "UPDATE books SET piece_of_books='$updatePieceOfBooks' WHERE isbn_number = '".$_POST['isbnnumber'][0]."'";//print_r($sql);exit;
                        $count = $conn->exec($sql);
                        ...
                        $sql = "UPDATE book_issues SET user_id='$userid', issue_details='$serializedPushedData' WHERE user_id = '$userid'";//print_r($sql);exit;
                        $count = $conn->exec($sql);
...
//print_r($totalPrevIssuedBooks);exit();
            ...
            if($attemptIssueBooks== 3){ // IF Want 3 book since I've 2 left
                header("Location:bookissue-teacher.php?book-issue-available-2=1");
...
//print_r($_POST["isbnnumber"][1]);exit;
                ...
                if($matchedCount>0){
                    ...
                    if ($lateFine >= 5) {
                        ...
                        $sql = "UPDATE book_issues SET user_id='$userid', issue_details='$serializedNextPushedData' WHERE user_id = '$userid'";//print_r($sql);exit;
                        $count = $conn->exec($sql);
                        header("Location:bookissue-manage.php?success=1");
            ...
            $sql = "INSERT INTO `book_issues` (`issue_date`, `user_id`, `name`, `issue_details`, `created_at`, `updated_at`)VALUES('$issue_date','$userid', '$teachername', '$serialized', now(), now())";//print_r($sql);exit;
            $count = $conn->exec($sql);
            header("Location:bookissue-manage.php?success=1");
            /* }*/
        }
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="pull-right">
                <a href="book-issue.php"><button class="btn btn-success">Book issue to student</button></a>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="box col-md-12">
            <div class="box-inner">
                <div class="box-header well" data-original-title="">
                    <h2><i class="glyphicon glyphicon-edit"></i>&nbsp;Issue book  to teacher</h2>

                </div>
                <div class="box-content">
                    <form role="form" action="bookissue-teacher.php" method="post">
                        <div class="form-group">
                            <span class="required">*</span><label for="exampleInputEmail1">Дата видачі</label>
                        ...
                        <div class="form-group">
                            <span class="required">*</span><label for="exampleInputEmail">Викладач ID</label>
                            <input type="text" name="" class="auto  form-control" id="emailid" placeholder="Email" style="width:20%">
                            <input type="hidden" id="hidden_auto_complete_teacherEmail"  name="teacherEmail">
                            <span style="color:red;"><?php echo @$message_error;?></span>

                        </div>
                        <div class="form-group">
                            <span class="required">*</span><label for="exampleInputEmail">Ім'я викладача</label>
                            <input type="text" class="form-control teachername" placeholder="Ім'я викладача" name="teachername" style="width:20%">
 ...
 $stmt = $conn->prepare("SELECT id,first_name,last_name,roll_no FROM users INNER JOIN students ON users.id=students.user_id WHERE roll_no=".$stid);
        ...
        while($row = $stmt->fetch()) {
         ...
         //echo json_encode(array('first_name'=>$return_arr['first_name'], 'last_name'=>$return_arr['last_name']));
                        ...
                        ?>
                        <!-- <div class="form-group">
                        <span class="required">*</span><label for="exampleInputEmail">Ім'я студента</label>
                      <input type="text" name="stname" class="form-control" id="stname" placeholder="Ім'я студента" style="width:40%">
                        <span style="color:red;"><?php echo @$message_error;?></span>
                    </div> -->
                        <div class="alert alert-info form-inline">
                            <a href="#" id="" class="addAnother" style="margin-left: 280px;"><i class="glyphicon glyphicon-plus-sign"></i>&nbsp;Додати ще один</a>
                            <div class="input-group col-md-12 visibleRow">

                                <div class="row copyitem">
                                    <div class="input-group col-md-4" style=" margin-left: 130px;">
                                        <span class="input-group-addon">ID книги</span>
                                        <input type="text" class="form-control isbnnumber" placeholder="" name="isbnnumber[]" id="" value="">
                                        <span style="color:red;"><?php echo @$message_error;?></span>
                                    </div>

                                    <div class="input-group col-md-4">
                                        <span class="input-group-addon fetch"  style="background: #aaeebb;border: 1px solid #eeccdd;" id="" >Принести</span>
                                    ...
                                    <button type="button" style="float:none" class="close" data-dismiss="alert"><span class="deleteItem" style="font-size: 19px;color:#2fa4e7">� Видалити</span></button>
                                </div>
                            ...
                            <div class="input-group col-md-12 visibleOne" style="display:none">
                                ...
                                <div class="row copyitem">
                                    ...
                                    <div class="input-group col-md-4">
                                        <span class="input-group-addon fetch1"  style="background: #aaeebb;border: 1px solid #eeccdd;" id="" >Принести</span>
                            ...
                            <div class="input-group col-md-12 visibleTwo" style="display:none">
                                ...
                                <div class="row copyitem">
                                    ...
                                    <div class="input-group col-md-4">
                                        <span class="input-group-addon fetch2"  style="background: #aaeebb;border: 1px solid #eeccdd;" id="" >Принести</span>
                        ...
                        <a href="bookissue-manage.php"><button type="button" class="btn btn-default" name="submit">Скасувати</button></a>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            $(".auto.form-control").autocomplete({
                source: "teacher-search.php",
                minLength: 1,
                select: function(event, ui) {
                    //alert(ui.item.value);
                    $("#hidden_auto_complete_teacherEmail").val(ui.item.value);
                    var emailID = ui.item.value;//alert(emailID);


                    $.get('teacher-search.php?email=' + emailID,
                        function(res) {//alert("firstname:"+res.fname+" lastname:"+res.lname);
                            $(".teachername").val(res.fname+" "+res.lname)
                            $(".userid").val(res.id)
            ...
            $("a.addAnother").click(function()
            {
                ...
                /*var new_row='<div class="row copyitem">'+
                 ...
                 '<div class="input-group col-md-4">'+
                 '<span class="input-group-addon fetch'+count+'" id="" style="background: #aaeebb;border: 1px solid #eeccdd">Принести</span>'+
                 '<input type="text" class="form-control bookname'+count+'" placeholder="" name="bookname" id="">'+
                 '</div>'+
                 '<button type="button" style="float:none" class="close" data-dismiss="alert">'+'<span style="font-size: 19px">� Видалити</span>'+'</button>'+
                 '</div>';
                 //alert(new_row);
